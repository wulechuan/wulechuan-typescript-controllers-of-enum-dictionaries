...
const 诸字典之构建配置总集之列表: 范_字典机_构建时所用配置总集_对外的准用式<
    范_本应用中一切字典之字典条目之原式,
    范_本应用中一切字典之字典条目之视式
>[] = [
    {
...
const 字典总机 = 构建字典群之总机<范_本应用中一切字典之字典条目之原式, 范_本应用中一切字典之字典条目之视式>({
    第一批字典机构建时所用配置总集_其列表: 诸字典之构建配置总集之列表,
...
export default class Page示范页1 extends Vue {
    ...
    private 字典总机: 范_本应用中一切字典群之总机 = 字典总机



    private 字典机_字典甲 (): null | 范_本应用中一切字典机 { return this.字典总机.为.取某字典机('字典甲') }
    private 字典机_字典乙 (): null | 范_本应用中一切字典机 { return this.字典总机.为.取某字典机('字典乙') }
    private 字典机_字典丙 (): null | 范_本应用中一切字典机 { return this.字典总机.为.取某字典机('字典丙') }

    private 字典条目之视式之列表_字典甲 (): 范_本应用中一切字典之字典条目之视式[] { return this.字典总机.为.取某字典诸条目之视式_其列表('字典甲') }
    private 字典条目之视式之列表_字典乙 (): 范_本应用中一切字典之字典条目之视式[] { return this.字典总机.为.取某字典诸条目之视式_其列表('字典乙') }
    private 字典条目之视式之列表_字典丙 (): 范_本应用中一切字典之字典条目之视式[] { return this.字典总机.为.取某字典诸条目之视式_其列表('字典丙') }

    private 当某表单项之值变动后 (所涉填写项之称谓: string, 新值: string, 所涉字典机: undefined | 范_本应用中一切字典机): void {
        let 消息: string

        if (!所涉字典机) {
            消息 = `填写项【${所涉填写项之称谓}】幕后的字典已不存在。`
        } else {
            const 新值之称谓 = 所涉字典机.为.凭某条目之值求其呈示称谓(新值)
            消息 = `填写项【${所涉填写项之称谓}】的取值变动了。变为：“${新值之称谓}”，对应的值为 “${新值}” 。`
        }
