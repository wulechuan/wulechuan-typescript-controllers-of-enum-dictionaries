...
> (
    ...
    */
    ...
    if (!是合规的字典群之总机之实例(该字典所属字典群之总机)) {
        console.error(
            `构建某【字典机】时，第一个实参（【\`该字典所属字典群之总机\`】）不合规。合规的第一实参应为【\`范_字典群之总机<this, T1, T2>\`】。\n   `,
            '给出的第一个实参为：', 该字典所属字典群之总机
        )
    ...
    if (
        ...
        !任何给定的字典_构建诸条目内容映射表之各色方案.includes(该字典构建诸字典条目内容映射表之方案)
    ) {
        该字典构建诸字典条目内容映射表之方案 = 任何给定的字典_构建诸条目内容映射表之默认方案
    }
    ...
    if (typeof 构建该字典所有条目之原式 !== 'function') {
        console.error(`${$_该字典机_消息前缀} 在用以构建该【字典机】的【形制要求总集】中：【\`构建该字典所有条目之原式\`】不是函数。`)
        return null
    }
    ...
    const 还应持有 = 求合规的非列表对象_若不合规则给出空的对象<范_该字典_范之零件_所持_形参准用式>(
        已知合规的形制要求总集['还应持有']
    )
    ...
    if (字典特别条目_其代表全选者_形参准用式) {
        ...
        if (!是合规的非列表对象(字典条目之视式)) {
            ...
            字典特别条目_其代表全选者_形参准用式.当正常候选项个数为零时仍允许采用本特别项 = !!当正常候选项个数为零时仍允许采用本特别项
        }
    }
    ...
    const _该字典机_凭该字典诸条目之原式_构建该字典诸条目之视式 = (): void => {
        ...
        const { 应构建字典条目之视式之哈希表 } = 该字典机.形制杂项集
        ...
        const 应采用代表全选之特别条目 = !!字典特别条目_其代表全选者 && (
            ...
            字典特别条目_其代表全选者.当正常候选项个数为零时仍允许采用本特别项
        )
    ...
    const _该字典机_按需构建诸字典条目内容映射表 = (): Promise<void> => {
        ...
        if (
            ...
            if (该字典构建诸字典条目内容映射表之方案 === '脱离该字典之诸条目原式之集而构建，仅一次即可' && 该字典诸字典条目内容映射表业已构建完毕) {
                ...
                if (该字典构建诸字典条目内容映射表之方案 === '脱离该字典之诸条目原式之集而构建，且须反复构建') {
            ...
            构建诸字典条目内容映射表之主体一事已落实 = 脱离字典条目原式之集构建诸字典条目内容映射表.call(
                this
            ).then(字典条目诸内容映射表之集 => {
                所持.字典条目诸内容映射表之集 = 字典条目诸内容映射表之集
                态.该字典诸字典条目内容映射表业已构建完毕 = true
    ...
    const _该字典机_重建数据 = (...参数表_用以获取字典诸条目之原式: any[]): Promise<void> => {
        ...
        态.期待_该字典最末一次数据构建任务已落定 = 期待_本次重建数据完毕
    ...
    const $_该字典机_发布消息_数据已重建一次 = (): void => {
        ...
        if (typeof 该字典每次数据重建后 === 'function') {
            该字典每次数据重建后.call(this, 该字典所属字典群之总机, 该字典机)
        }
