...
> {
    ...
    let $_该字典群之总机_期待_迄今一切在行批次之任务均已落定且善后_落实之: undefined | 范_某期待_用以落实之之函数<void>
    let $_该字典群之总机_期待_迄今一切在行批次之任务均已落定且善后_落空之: undefined | 范_某期待_用以落空之之函数<void>
    let $_该字典群之总机_期待_迄今一切在行批次之任务均已落定且善后: null | Promise<void>
    let $_该字典群之总机_迄今一切在行批次之任务之诸期待之去重后的列表: Array<Promise<void>> = []
    let $_该字典群之总机_迄今一切在行批次之任务中遭遇的所有异常_其列表: Array<any>;





    const _该字典群之总机_制作一批新字典 = <
    ...
    >(
        ...
        一批字典机之形制要求总集之列表.forEach(_该字典群之总机_按需制作并追加单个字典<
            范_该批字典_条目原式_可能的共同形态,
            范_该批字典_内所有字典_条目视式_各色形态之交集
        >)

        return 该字典群之总机.为.令一批字典各自重建数据('仅限那些允许参与数据自动构建任务且尚无构建好之数据的字典参与')
    }



    const _该字典群之总机_按需制作并追加单个字典 = <
    ...
    const _该字典群之总机_令一批字典各自重建数据 = (
        ...
        if (
            ...
            !任何给定的字典群之总机_成批重建字典数据之各色方案.includes(本次执行须为哪些字典重建数据)
        ) {
            本次执行须为哪些字典重建数据 = '仅限那些允许参与数据自动构建任务且尚无构建好之数据的字典参与'
        }
        ...
        if (本次执行须为哪些字典重建数据 === '仅限那些允许参与数据自动构建任务且尚无构建好之数据的字典参与') {
            须为其构建数据之字典机之列表 = 须为其构建数据之字典机之列表.filter(
                某字典机 => !某字典机.形制杂项集.该字典在默认情形下不参与字典群总机发起的数据重建任务 && !某字典机.态.该字典当下所持的数据系由某次构建而得
            )
        } else if (本次执行须为哪些字典重建数据 === '所有允许参与数据自动构建任务的字典均参与，不论其是否已构建好数据') {
            须为其构建数据之字典机之列表 = 须为其构建数据之字典机之列表.filter(
                某字典机 => !某字典机.形制杂项集.该字典在默认情形下不参与字典群总机发起的数据重建任务
            )
        }
        ...
        const 本批次各任务落定之期待_其列表: Array<Promise<void>> = 须为其构建数据之字典机之列表.map(
            某字典机 => 某字典机.为.重建数据()
        )

        const 本批次诸任务是否同时出现在以往批次中_其列表: Array<boolean> = 本批次各任务落定之期待_其列表.map(某任务落定之期待 => {
            const 该任务确系在多批次中重复出现者 = $_该字典群之总机_迄今一切在行批次之任务之诸期待之去重后的列表.includes(某任务落定之期待)

            if (该任务确系在多批次中重复出现者) {
                该总机的调试信息之详尽等级 >= 2 && console.debug(`${本批次任务之消息前缀} 发现由多批次共用的任务（发现复用的【期待】）：`, 某任务落定之期待)
            } else {
                $_该字典群之总机_迄今一切在行批次之任务之诸期待之去重后的列表.push(某任务落定之期待)
            }

            return 该任务确系在多批次中重复出现者
        })

        const 期待_本批次中各任务均已落定: Promise<void> = Promise.allSettled(
            本批次各任务落定之期待_其列表
        )
            .then((一切期待之结果之列表) => {
                const 本批次任务中一切异常之列表: any[] = []

                一切期待之结果之列表.forEach((某期待之结果, 列表编号) => {
                    const 该期待所涉字典机 = 须为其构建数据之字典机之列表[列表编号]
                    const 该任务在本批次和以往在行批次均出现者 = 本批次诸任务是否同时出现在以往批次中_其列表[列表编号]

                    if (某期待之结果.status === 'rejected') {
                        const { reason: 该期待落空之原因 } = 某期待之结果

                        if (!该任务在本批次和以往在行批次均出现者) {
                            该总机的调试信息之详尽等级 >= 1 && console.warn(`某批次数据重建任务：【字典】“${该期待所涉字典机.该字典之唯一标识}” 异常。`, 该期待落空之原因)
                            $_该字典群之总机_迄今一切在行批次之任务中遭遇的所有异常_其列表.push(该期待落空之原因)
                        }

                        本批次任务中一切异常之列表.push(该期待落空之原因)
                    }
                })

                if (本批次任务中一切异常之列表.length > 0) {
                    return Promise.reject(本批次任务中一切异常之列表)
                }
            })

        const 期待_本批次中各任务均已落定且异常已隐匿: Promise<void> = 期待_本批次中各任务均已落定.catch(() => {})



        const 发起本批次任务之前不存在任何未落定的以往批次: boolean = !$_该字典群之总机_期待_迄今一切在行批次之任务均已落定且善后



        let 期待_本批次加之其之前一切在行批次之任务均已落定: Promise<void>

        if (发起本批次任务之前不存在任何未落定的以往批次) {
            $_该字典群之总机_迄今一切在行批次之任务中遭遇的所有异常_其列表 = []

            态.期待_迄今所有批次的字典机之数据重建任务均已落定 = new Promise((落实, 落空) => {
                const 善后 = () => {
                    态.当下仍有字典机在构建数据 = false
                    $_该字典群之总机_期待_迄今一切在行批次之任务均已落定且善后 = null
                    $_该字典群之总机_期待_迄今一切在行批次之任务均已落定且善后_落实之 = undefined
                    $_该字典群之总机_期待_迄今一切在行批次之任务均已落定且善后_落空之 = undefined
                    $_该字典群之总机_迄今一切在行批次之任务之诸期待之去重后的列表 = []
                    态.期待_迄今所有批次的字典机之数据重建任务均已落定 = Promise.resolve()
                }

                $_该字典群之总机_期待_迄今一切在行批次之任务均已落定且善后_落实之 = () => {
                    落实(undefined)
                    善后()
                }

                $_该字典群之总机_期待_迄今一切在行批次之任务均已落定且善后_落空之 = () => {
                    落空($_该字典群之总机_迄今一切在行批次之任务中遭遇的所有异常_其列表)
                    善后()
                }
            })
                // .then(() => {})
            ...
            期待_本批次加之其之前一切在行批次之任务均已落定 = 期待_本批次中各任务均已落定且异常已隐匿
        } else {
            期待_本批次加之其之前一切在行批次之任务均已落定 = Promise.allSettled([
                $_该字典群之总机_期待_迄今一切在行批次之任务均已落定且善后,
                期待_本批次中各任务均已落定且异常已隐匿,
            ])
                .then(() => {})
                // .catch(() => {})
        }
        ...
        const 期待_本批次加之其之前一切在行批次之任务均已落定且已善后: Promise<void> = 期待_本批次加之其之前一切在行批次之任务均已落定
            .then(() => {
                该总机的调试信息之详尽等级 >= 2 && console.debug(`${本批次任务之消息前缀} 刚刚完成某批次数据重建任务。`)

                if ($_该字典群之总机_期待_迄今一切在行批次之任务均已落定且善后 !== 期待_本批次加之其之前一切在行批次之任务均已落定且已善后) {
                    该总机的调试信息之详尽等级 >= 2 && console.debug(`${本批次任务之消息前缀} 仍有某批次数据重建任务未完成。`)
                    return
                }

                该总机的调试信息之详尽等级 >= 1 && console.debug(`${本批次任务之消息前缀} 所有迄今所有批次的数据重建任务均已落定。`)

                if ($_该字典群之总机_迄今一切在行批次之任务中遭遇的所有异常_其列表.length === 0) {
                    if (typeof $_该字典群之总机_期待_迄今一切在行批次之任务均已落定且善后_落实之 === 'function') {
                        $_该字典群之总机_期待_迄今一切在行批次之任务均已落定且善后_落实之()
                    }
                } else {
                    if (typeof $_该字典群之总机_期待_迄今一切在行批次之任务均已落定且善后_落空之 === 'function') {
                        $_该字典群之总机_期待_迄今一切在行批次之任务均已落定且善后_落空之()
                    }
                }
            })
        ...
        $_该字典群之总机_期待_迄今一切在行批次之任务均已落定且善后 = 期待_本批次加之其之前一切在行批次之任务均已落定且已善后



        return 期待_本批次中各任务均已落定
    }
    ...
    该字典群之总机.为.制作一批新字典(总机之已知合规的形制要求总集.第一批字典机形制要求总集_其列表)
