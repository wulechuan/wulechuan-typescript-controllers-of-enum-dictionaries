...
> {
    ...
    const _该字典群之总机_令所有字典按需重建数据 = (
        ...
        const 期待_本批次任务均已完成: Promise<void> = Promise.allSettled(
            ...
            .then((一切期待之结果之列表) => {
                ...
                一切期待之结果之列表.forEach((某期待之结果, 列表编号) => {
                    ...
                    if (某期待之结果.status === 'rejected') {
                        ...
                        $_该字典群之总机_迄今所有批次任务中遭遇的所有异常_其列表.push(该期待落空之原因)
                    }
                })
        ...
        const 期待_本批次任务均已落定: Promise<void> = 期待_本批次任务均已完成.catch(() => {})



        const 发起本批次任务之前没有未完成的以往批次: boolean = !$_该字典群之总机_期待_迄今一切在行批次之任务均已落定且善后



        let 期待_含本批次任务加之其之前所有批次的任务均已落定: Promise<void>
        ...
        if (发起本批次任务之前没有未完成的以往批次) {
            ...
            态.期待_迄今所有批次的字典机之数据重建任务均已完成 = new Promise((落实, 落空) => {
                ...
                $_该字典群之总机_期待_所有批次的数据重建任务均已完毕_落空之 = () => {
                    落空($_该字典群之总机_迄今所有批次任务中遭遇的所有异常_其列表)
                    善后()
                }
            })
                // .then(() => {})
            ...
            期待_含本批次任务加之其之前所有批次的任务均已落定 = 期待_本批次任务均已落定
        } else {
            期待_含本批次任务加之其之前所有批次的任务均已落定 = Promise.allSettled([
        ...
        const 期待_含本批次任务加之其之前所有批次的任务均已落定且已善后: Promise<void> = 期待_含本批次任务加之其之前所有批次的任务均已落定
            .then(() => {
                该总机的调试信息之详尽等级 >= 2 && console.debug(`${本批次任务之消息前缀} 刚刚完成某批次数据重建任务。`)

                if ($_该字典群之总机_期待_迄今一切在行批次之任务均已落定且善后 !== 期待_含本批次任务加之其之前所有批次的任务均已落定且已善后) {
        ...
        $_该字典群之总机_期待_迄今一切在行批次之任务均已落定且善后 = 期待_含本批次任务加之其之前所有批次的任务均已落定且已善后
