...
> {
    ...
    type 范_该字典群之总机_诸事之应对_准用式 = 范_字典群之总机_范之零件_诸事之应对_准用式<
        范_该总机_This,
        范_该总机_内所谓标准字典_条目原式,
        范_该总机_内所有字典_条目视式_各色形态之交集
    >;
    ...
    const _该字典群之总机_追加一批字典 = <范_该组字典_条目原式_可能的默认形态 = 范_该总机_内所谓标准字典_条目原式>(
    ...
    const _该字典群之总机_令所有字典按需重建数据 = (
        ...
        // console.debug(本批次任务之消息前缀, '须为其构建数据之字典机之列表', 须为其构建数据之字典机之列表.map(字典机 => 字典机.该字典之唯一标识))
    ...
    const 总机之已知合规的形制要求总集 = 求合规的非列表对象_若不合规则给出空的对象<范_该字典群之总机_形制要求总集_准用式>(该字典群之总机之形制要求总集)
    ...
    const 该总机_诸事之应对 = 求合规的非列表对象_若不合规则给出空的对象<范_该字典群之总机_诸事之应对_准用式>(
    ...
    const 该字典群之总机: 范_该字典群之总机 = {
        ...
        为: {
            追加一批字典 (一批字典机之形制要求总集之列表) {
                return _该字典群之总机_追加一批字典(一批字典机之形制要求总集之列表)
            },

            删除一批字典 (欲删除之诸字典之唯一标识之列表) {
                return _该字典群之总机_删除一批字典(欲删除之诸字典之唯一标识之列表)
            },

            取某字典机 (欲索取之字典_其唯一标识) {
                return _该字典群之总机_取某字典机(欲索取之字典_其唯一标识)
            },

            取某字典诸条目之视式_其列表 (所涉字典_其唯一标识) {
                return _该字典群之总机_取某字典诸条目之视式_其列表(所涉字典_其唯一标识)
            },

            取某字典诸条目之视式_其哈希表 (所涉字典_其唯一标识) {
                return _该字典群之总机_取某字典诸条目之视式_其哈希表(所涉字典_其唯一标识)
            },

            令所有字典按需重建数据 (本次执行须筛选参与之诸字典时应采取哪种筛选方案) {
                return _该字典群之总机_令所有字典按需重建数据(本次执行须筛选参与之诸字典时应采取哪种筛选方案)
            },
        },
