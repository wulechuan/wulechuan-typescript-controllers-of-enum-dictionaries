...
declare module '@wulechuan/controllers-of-enum-dictionaries' {
     ...
     * 则本工具集产生的【字典机】中的各【方法函数】均会被 Vuejs
     * 自动绑定 this 。
     * 以 `字典机.应对.某事件处理程序` 为例，
     ...
     * 同时，令【准用式】的 `诸事件之应对` 中的各【方法函数】配有绑定的 this 。
     *
     * 另一做法我也尝试过，可行，但最终放弃。
     * 该被放弃之做法如下：
     *
     * 在【范】之定义时，【完备式】、【准用式】中的各【方法函数】均配有绑定 this 。
    ...
    export type 范_字典机_范之零件_形制要求杂集_准用式<
    ...
    > = {
        ...
        该字典每次数据重建后?: (
            该字典机: 范_字典机<范_该字典机_This, 范_该字典_条目原式, 范_该字典_条目视式>
        ) => void;
    };



    export type 范_字典机_范之零件_诸事之应对_准用式<
        范_该字典机_This = 范_字典机系统_This之默认式,
        范_该字典_条目原式 = any,
        范_该字典_条目视式 extends 范_字典条目之视式_最简式 = 范_字典条目之视式_最简式<any>
    > = {
    ...
    > = (
        ...
        所涉字典机: 范_字典机<
            范_所涉总机_This,
            范_所涉总机_所涉所谓标准字典_条目原式, // FIXME 此处原本明确写作 any 。
            范_所涉总机_内所有字典_条目视式_各色形态之交集
        >
    ) => void;

    export type 范_某字典群_内任何所谓标准字典_某次数据重建后_统一做法_未绑定This<
        范_所涉总机_所涉所谓标准字典_条目原式 = any,
        范_所涉总机_内所有字典_条目视式_各色形态之交集 extends 范_字典条目之视式_最简式 = 范_字典条目之视式_最简式<any>
    > = OmitThisParameter<
    ...
    export type 范_某字典群_内任何所谓标准字典_凭字典条目原式之集构建诸字典条目内容映射表_默认做法_未绑定This<
        范_所涉总机_所涉所谓标准字典_条目原式 = any,
        范_所涉总机_内所有字典_条目视式_各色形态之交集 extends 范_字典条目之视式_最简式 = 范_字典条目之视式_最简式<any>
    > = OmitThisParameter<
    ...
    export type 范_某字典群_内任何所谓标准字典_凭某条目之原式构建其视式_默认做法_未绑定This<
        范_所涉总机_所涉所谓标准字典_条目原式 = any,
        范_所涉总机_内所有字典_条目视式_各色形态之交集 extends 范_字典条目之视式_最简式 = 范_字典条目之视式_最简式<any>
    > = OmitThisParameter<
