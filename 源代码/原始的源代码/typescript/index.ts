    ...
    >(
        ...
        function _按需构建诸字典条目内容映射表 (): Promise<void> {
            ...
            if (该字典构建诸字典条目内容映射表之方案 === '从该字典条目之原式之集提取而得') {
                ...
                if (!字典诸条目之原式_其列表) {
                    console.error(`字典 “${该字典之唯一标识}” 的诸【字典条目内容映射表】配置为 “${该字典构建诸字典条目内容映射表之方案}”。但构建诸【字典条目内容映射表】时，发现【\`字典诸条目之原式_其列表\`】尚不可用。`)
                    return 构建诸字典条目内容映射表一事已落实
                    // return Promise.reject()
                }

                if (typeof 凭字典条目原式之集构建诸字典条目内容映射表 !== 'function') {
                    console.error(`字典 “${该字典之唯一标识}” 的诸【字典条目内容映射表】配置为 “${该字典构建诸字典条目内容映射表之方案}”。但构建诸【字典条目内容映射表】时，发现【\`凭字典条目原式之集构建诸字典条目内容映射表\`】不是函数。`)
                    return 构建诸字典条目内容映射表一事已落实
                    // return Promise.reject()
                }

                return 凭字典条目原式之集构建诸字典条目内容映射表(字典诸条目之原式_其列表).then(字典条目内容映射表集 => {
            ...
            if (
                ...
                if (typeof 脱离字典条目原式之集构建诸字典条目内容映射表 !== 'function') {
                    console.error(`字典 “${该字典之唯一标识}” 的诸【字典条目内容映射表】配置为 “${该字典构建诸字典条目内容映射表之方案}”。但构建诸【字典条目内容映射表】时，发现【\`脱离字典条目原式之集构建诸字典条目内容映射表\`】不是函数。`)
                    return 构建诸字典条目内容映射表一事已落实
                    // return Promise.reject()
                }

                return 脱离字典条目原式之集构建诸字典条目内容映射表().then(字典条目内容映射表集 => {
                    数据集.字典条目内容映射表集 = 字典条目内容映射表集
                    态.该字典诸字典条目内容映射表业已构建完毕 = true
                })
            }
