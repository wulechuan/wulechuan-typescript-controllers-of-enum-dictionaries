...
> = {
    ...
    配置集: {
        代表全选的字典条目?: null | 范_某字典_代表全选的字典条目_准用形式<范_该字典_条目视式>;
        代表全都不选的字典条目?: null | 范_某字典_代表全都不选的字典条目_准用形式<范_该字典_条目视式>;
    ...
    事件处理程序集: {
        ...
        脱离该字典条目之原式之集构建诸数据映射表?: () => Promise<{
            从呈示称谓至值: 范_数据映射表_从呈示称谓至值<范_该字典_条目视式['值']>;
            从值至呈示称谓: 范_数据映射表_从值至呈示称谓;
        }>;

        凭该字典条目之原式之集构建诸数据映射表?: (
            该字典_条目原式集: Array<范_该字典_条目原式>
        ) => Promise<{
            从呈示称谓至值: 范_数据映射表_从呈示称谓至值<范_该字典_条目视式['值']>;
            从值至呈示称谓: 范_数据映射表_从值至呈示称谓;
        }>;
...
> = {
    ...
    数据集: {
        ...
        字典条目之视式集: {
            列表形式: 范_字典条目之视式_之列表<范_该字典_条目视式>;
            哈希表形式: 范_字典条目之视式_之哈希表<范_该字典_条目视式>;
        };
...
export type 范_用以存诸字典机之哈希表<
    范_该组字典_条目原式_可能的默认形态 = any,
    范_该组字典_条目视式_各形态之交集 extends 范_字典条目之视式_最简式 = 范_字典条目之视式_最简式<any, any>
> = {
    [字典之唯一标识: string]: 范_字典机<范_该组字典_条目原式_可能的默认形态, 范_该组字典_条目视式_各形态之交集>;
};
...
> = (
    该字典_条目原式集: Array<范_某次调用期间所操作之字典_条目原式_默认形态>
) => Promise<{
...
> = (
    该字典_某字典条目之原式: 范_某次调用期间所操作之字典_条目原式_默认形态
...
> = {
    ...
    最初的一批用以构建字典机的配置集之列表: Array<
        范_字典机_构建时所用配置总集_对外的准用形式<any, 范_该总机内所有字典_条目视式_各形态之交集>
    >;
...
> = {
    ...
    为: {
        ...
        追加一批字典: <范_该组字典_条目原式_可能的默认形态 = 范_该总机内所有字典_条目原式_可能的默认形态>(
            用以构建一批字典机之配置集之列表?: Array<
                范_字典机_构建时所用配置总集_对所属总机的准用形式<范_该组字典_条目原式_可能的默认形态, 范_该总机内所有字典_条目视式_各形态之交集>
            >
        ) => void;
    ...
    _暗为: {
        _按需追加单个字典: <
            ...
            范_该字典_条目视式 extends 范_该总机内所有字典_条目视式_各形态之交集 = 范_该总机内所有字典_条目视式_各形态之交集
        >(
            用以构建该字典机之配置总集?: 范_字典机_构建时所用配置总集_对外的准用形式<范_该字典_条目原式, 范_该字典_条目视式>
        ) => (null | 范_字典机<范_该字典_条目原式, 范_该字典_条目视式>);
...
> {
    ...
    function 追加一批字典 <范_该组字典_条目原式_可能的默认形态 = 范_该总机内所有字典_条目原式_可能的默认形态>(
        用以构建一批字典机之配置集之列表?: Array<
            范_字典机_构建时所用配置总集_对外的准用形式<范_该组字典_条目原式_可能的默认形态, 范_该总机内所有字典_条目视式_各形态之交集>
        >
    ): void {
    ...
    function 构建单个字典机 <
        ...
        范_该字典_条目视式 extends 范_该总机内所有字典_条目视式_各形态之交集 = 范_该总机内所有字典_条目视式_各形态之交集
    >(
        用以构建该字典机之配置总集?: 范_字典机_构建时所用配置总集_对所属总机的准用形式<范_该字典_条目原式, 范_该字典_条目视式>,

        默认值集?: {
            任何字典_凭该字典条目之原式之集构建诸数据映射表_默认做法?: 范_任何字典_凭该字典条目之原式之集构建诸数据映射表_默认做法<范_该总机内所有字典_条目原式_可能的默认形态, 范_该总机内所有字典_条目视式_各形态之交集['值']>;
        ...
        /** 此处定义该【范】是为了在本函数体中方便指代。 */
        type 范_该字典_代表全选的字典条目_准用形式 = 范_某字典_代表全选的字典条目_准用形式<范_该字典_条目视式>;

        /** 此处定义该【范】是为了在本函数体中方便指代。 */
        type 范_该字典_代表全都不选的字典条目_完备式 = 范_某字典_代表全都不选的字典条目_完备式<范_该字典_条目视式>;

        /** 此处定义该【范】是为了在本函数体中方便指代。 */
        type 范_该字典_代表全都不选的字典条目_准用形式 = 范_某字典_代表全都不选的字典条目_准用形式<范_该字典_条目视式>;
        ...
        if (外界须提供函数用以凭该字典条目之原式之集构建诸数据映射表) {
            if (typeof 凭该字典条目之原式之集构建诸数据映射表 !== 'function') {
                if (typeof 任何字典_凭该字典条目之原式之集构建诸数据映射表_默认做法 === 'function') {
                    凭该字典条目之原式之集构建诸数据映射表 = 任何字典_凭该字典条目之原式之集构建诸数据映射表_默认做法 as unknown as 范_任何字典_凭该字典条目之原式之集构建诸数据映射表_默认做法<范_该字典_条目原式, 范_该字典_条目视式['值']>
                } else {
