...
type 范_字典机_构建时所用配置集之零件_事件处理程序集<
    范_该字典_条目原式 = any,
    范_该字典_条目视式 extends 范_字典条目之视式_最简式 = 范_字典条目之视式_最简式<any>,
> = {
    构建或获取该字典所有条目之原式: (
...
> = {
    ...
    配置杂集:
        范_字典机_构建时所用配置集之零件_配置集甲<
            范_该字典_条目原式, 范_该字典_条目视式
        >;

    事件处理程序集:
        范_字典机_构建时所用配置集之零件_事件处理程序集<
            范_该字典_条目原式, 范_该字典_条目视式
        >;
};
...
> {
    ...
    >(
        ...
        默认值集?: {
            任何字典_凭字典条目原式之集构建诸字典条目内容映射表_默认做法?: 范_任何字典_凭字典条目原式之集构建诸字典条目内容映射表_默认做法<范_该总机内所有字典_条目原式_可能的默认形态, 范_该总机内所有字典_条目视式_各色形态之交集['值']>;
            任何字典_凭某条目之原式构建其视式_默认做法?: 范_任何字典_凭某条目之原式构建其视式_默认做法<范_该总机内所有字典_条目原式_可能的默认形态, 范_该总机内所有字典_条目视式_各色形态之交集>;
        }
    ): (
        null | 范_字典机<范_该字典_条目原式, 范_该字典_条目视式>
    ) {
        ...
        const 配置杂集 = 求合规的非列表对象_若不合规则给出空的对象<范_用以构建该字典机之配置总集_完备式['配置杂集']>(已知合规的配置总集['配置杂集'])
        ...
        const 外界须提供函数用以凭字典条目原式之集构建诸字典条目内容映射表 = 该字典构建诸字典条目内容映射表之方案 === '从该字典条目之原式之集提取而得'
        const 外界须提供函数用以脱离字典条目原式之集构建诸字典条目内容映射表 = ([
            '脱离该字典条目之原式之集而构建，仅一次即可',
            '脱离该字典条目之原式之集而构建，且须反复获取或构建',
        ...
        const 该字典并不参与字典群总机发起的所谓数据自动构建任务 = !!配置杂集.该字典并不参与字典群总机发起的所谓数据自动构建任务
        const 应构建字典条目之视式之哈希表 = !!配置杂集.应构建字典条目之视式之哈希表



        let 代表全选的字典条目_准用式 = 求合规的非列表对象_若不合规则给出null<范_该字典_代表全选的字典条目_准用式>(配置杂集['代表全选的字典条目'])
        if (代表全选的字典条目_准用式) {
        ...
        let 代表全都不选的字典条目_准用式 = 求合规的非列表对象_若不合规则给出null<范_该字典_代表全都不选的字典条目_准用式>(配置杂集['代表全都不选的字典条目'])
        if (代表全都不选的字典条目_准用式) {
        ...
        function _按需构建诸字典条目内容映射表 (): Promise<void> {
            ...
            if (该字典构建诸字典条目内容映射表之方案 === '从该字典条目之原式之集提取而得') {
                ...
                return 凭字典条目原式之集构建诸字典条目内容映射表(字典条目之原式集).then(字典条目内容映射表集 => {
        ...
        function 重建数据 (...用以获取字典条目之原式集的参数表: any[]): Promise<void> {
            ...
            return Promise.all([
                ...
                该字典机._事件处理程序集.构建或获取该字典所有条目之原式(...用以获取字典条目之原式集的参数表).then(字典条目之原式集 => {
