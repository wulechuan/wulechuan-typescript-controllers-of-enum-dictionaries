...
> {
    ...
    function 令所有字典按需重建数据 (
        ...
        if (当下仍有字典机在构建数据) {
            ...
            态.当下仍有字典机在构建数据 = true
            期待_迄今所有批次的字典机之数据重建任务均已完成 = 期待_此批字典机之数据重建任务均已完成.then(() => {
                态.当下仍有字典机在构建数据 = false
            })
        }
    ...
    >(
        ...
        function 重建数据 (...用以获取字典诸条目之原式_参数表: any[]): Promise<void> {
            const { 该字典之唯一标识, 配置杂集, 态, 为 } = 该字典机

            if (态.当下正在构建数据) {
                console.debug(`字典机【${该字典之唯一标识}】已经在重建数据，且该重建数据之任务尚未完毕。字典机不会并行多个数据重建任务。`)
            ...
            态.当下正在构建数据 = true
            console.debug(`字典机【${该字典之唯一标识}】开始重建数据。`)
