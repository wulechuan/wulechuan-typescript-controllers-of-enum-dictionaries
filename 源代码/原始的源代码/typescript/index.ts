...
export interface 范_数据视项_最简式<范_项值 = any, 范_负载 = any> {
    唯一标识: 范_字典项之唯一标识;
    在界面中的称谓: string;
    值: 范_项值;
    ...
    /**  为求实用，每个界面项可挂负载。例如，可将该【数据视项】对应的【数据原项】作为负载。 */
    负载?: 范_负载;
}



export type 范_数据视项_之列表<
    范_数据视项 extends 范_数据视项_最简式 = 范_数据视项_最简式<any, any>
> = Array<范_数据视项>;

export type 范_数据视项_之哈希表<
    范_数据视项 extends 范_数据视项_最简式 = 范_数据视项_最简式<any, any>
> = { [数据项之唯一标识: string]: 范_数据视项; };
...
export type 范_某字典_代表全选的字典项_完备形式<
    范_该字典_数据视项 extends 范_数据视项_最简式 = 范_数据视项_最简式<any, any>
> = {
    应位于候选列表之该处: '首部' | '尾部';
    数据视项: 范_该字典_数据视项;
};
...
> = {
    应位于候选列表之该处?: '首部' | '尾部';
    数据视项: Omit<范_该字典_数据视项, '在界面中的称谓'> & { 在界面中的称谓?: string; };
};
...
> = {
    ...
    配置项集: {
        代表全选的字典项?: null | 范_某字典_代表全选的字典项_准用形式<范_该字典_数据视项>;
        代表全都不选的字典项?: null | 范_某字典_代表全都不选的字典项_准用形式<范_该字典_数据视项>;
    ...
    事件处理程序集: {
        ...
        脱离数据原项集构建诸数据映射表?: () => Promise<{
            从界面措辞至值: 范_数据映射表_从界面措辞至值<范_该字典_数据视项['值']>;
        ...
        每次数据重建后?: (
            该字典机: 范_字典机<范_该字典_数据原项, 范_该字典_数据视项>
        ) => void;
...
> = {
    ...
    数据集: {
        代表全选的字典项: null | 范_某字典_代表全选的字典项_完备形式<范_该字典_数据视项>;
        代表全都不选的字典项: null | 范_某字典_代表全都不选的字典项_完备形式<范_该字典_数据视项>;

        数据原项集: null | Array<范_该字典_数据原项>;

        数据视项集: {
            列表形式: 范_数据视项_之列表<范_该字典_数据视项>;
            哈希表形式: 范_数据视项_之哈希表<范_该字典_数据视项>;
        };
...
export type 范_字典机之集之列表形式<
    ...
    范_该组字典_数据视项_各形态之交集 extends 范_数据视项_最简式 = 范_数据视项_最简式<any, any>
> = Array<
    范_字典机<范_该组字典_数据原项_可能的默认形态, 范_该组字典_数据视项_各形态之交集>
>;
...
export type 范_字典机之集之哈希表形式<
    ...
    范_该组字典_数据视项_各形态之交集 extends 范_数据视项_最简式 = 范_数据视项_最简式<any, any>
> = {
    [字典之唯一标识: string]: 范_字典机<范_该组字典_数据原项_可能的默认形态, 范_该组字典_数据视项_各形态之交集>;
};
...
export type 范_任何字典_凭某数据原项构建数据视项_默认做法<
    范_某次调用期间所操作之字典_数据原项_默认形态 = any,
    范_某次调用期间所操作之字典_数据视项_默认形态 extends 范_数据视项_最简式 = 范_数据视项_最简式<any, any>
> = (
    该字典_某数据原项: 范_某次调用期间所操作之字典_数据原项_默认形态
) => 范_某次调用期间所操作之字典_数据视项_默认形态;



export type 范_用以构建字典群之总机之配置项总集_完备形式<
    范_该总机内所有字典_数据原项_可能的默认形态 = any,
    范_该总机内所有字典_数据视项_各形态之交集 extends 范_数据视项_最简式 = 范_数据视项_最简式<any, any>
> = {
    不应自动善始: boolean;

    最初的一批用以构建字典机的配置项集之列表: Array<
        范_用以构建单个字典机之配置项总集_对外的准用形式<any, 范_该总机内所有字典_数据视项_各形态之交集>
    >;

    事件处理程序集: {
        任何字典_凭数据原项集构建诸数据映射表_默认做法?: 范_任何字典_凭数据原项集构建诸数据映射表_默认做法<
            范_该总机内所有字典_数据原项_可能的默认形态,
            范_该总机内所有字典_数据视项_各形态之交集['值']
        >;

        任何字典_凭某数据原项构建数据视项_默认做法?: 范_任何字典_凭某数据原项构建数据视项_默认做法<
            范_该总机内所有字典_数据原项_可能的默认形态,
            范_该总机内所有字典_数据视项_各形态之交集
        >;
        ...
        任何字典_某次数据重建后?: (
            ...
            所涉字典机: 范_字典机<any, 范_该总机内所有字典_数据视项_各形态之交集>
        ) => void;
    };
};
...
> = {
    ...
    为: {
        ...
        追加一批字典: <范_该组字典_数据原项_可能的默认形态 = 范_该总机内所有字典_数据原项_可能的默认形态>(
            用以构建一批字典机之配置项集之列表?: Array<
                范_用以构建单个字典机之配置项总集_对所属总机的准用形式<范_该组字典_数据原项_可能的默认形态, 范_该总机内所有字典_数据视项_各形态之交集>
            >
        ) => void;

        取某字典机: <
            范_该字典_数据原项 = 范_该总机内所有字典_数据原项_可能的默认形态,
            范_该字典_数据视项 extends 范_该总机内所有字典_数据视项_各形态之交集 = 范_该总机内所有字典_数据视项_各形态之交集
        >(
            欲索取之字典之唯一标识?: 范_字典之唯一标识
        ) => (null | 范_字典机<范_该字典_数据原项, 范_该字典_数据视项>);
    ...
    _暗为: {
        _按需追加单个字典: <
            ...
            范_该字典_数据视项 extends 范_该总机内所有字典_数据视项_各形态之交集 = 范_该总机内所有字典_数据视项_各形态之交集
        >(
            用以构建该字典机之配置项总集?: 范_用以构建单个字典机之配置项总集_对外的准用形式<范_该字典_数据原项, 范_该字典_数据视项>
        ) => (null | 范_字典机<范_该字典_数据原项, 范_该字典_数据视项>);
...
> {
    ...
    function 追加一批字典 <范_该组字典_数据原项_可能的默认形态 = 范_该总机内所有字典_数据原项_可能的默认形态>(
        用以构建一批字典机之配置项集之列表?: Array<
            范_用以构建单个字典机之配置项总集_对外的准用形式<范_该组字典_数据原项_可能的默认形态, 范_该总机内所有字典_数据视项_各形态之交集>
        >
    ): void {
    ...
    >(
        ...
        const 字典机 = 字典机之哈希表[已知合规的唯一标识] as 范_字典机<范_该字典_数据原项, 范_该字典_数据视项>
        return 字典机 || null
    }



    function 取某字典之数据视项之列表 <
        范_该字典_数据原项 = 范_该总机内所有字典_数据原项_可能的默认形态,
        范_该字典_数据视项 extends 范_该总机内所有字典_数据视项_各形态之交集 = 范_该总机内所有字典_数据视项_各形态之交集
    >(
        欲索取其中数据之字典之唯一标识?: 范_字典之唯一标识
    ): 范_数据视项_之列表<范_该字典_数据视项> {
        const 字典机 = 取某字典机<范_该字典_数据原项, 范_该字典_数据视项>(欲索取其中数据之字典之唯一标识)
        if (字典机) {
            return 字典机.数据集.数据视项集.列表形式
        } else {
            return []
        }
    }
    ...
    >(
        ...
        const 字典机 = 取某字典机<范_该字典_数据原项, 范_该字典_数据视项>(欲索取其中数据之字典之唯一标识)
        if (字典机 && 字典机.数据集.数据视项集.哈希表形式) {
            return 字典机.数据集.数据视项集.哈希表形式
        } else {
            return {}
        }
    }
    ...
    >(
        ...
        const 字典机 = 取某字典机<范_该字典_数据原项, 范_该字典_数据视项>(已知合规的唯一标识)
        if (字典机) {
        ...
        const 构建之结果 = 构建单个字典机<范_该字典_数据原项, 范_该字典_数据视项>(
            最终合并采纳的配置项总集,
    ...
    function _发布消息_单个字典之数据已重建 (所涉字典机: 范_字典机<范_该总机内所有字典_数据原项_可能的默认形态, 范_该总机内所有字典_数据视项_各形态之交集>): void {
    ...
    function 构建单个字典机 <
        ...
        范_该字典_数据视项 extends 范_该总机内所有字典_数据视项_各形态之交集 = 范_该总机内所有字典_数据视项_各形态之交集
    >(
        用以构建该字典机之配置项总集?: 范_用以构建单个字典机之配置项总集_对所属总机的准用形式<范_该字典_数据原项, 范_该字典_数据视项>,

        默认值集?: {
            任何字典_凭数据原项集构建诸数据映射表_默认做法?: 范_任何字典_凭数据原项集构建诸数据映射表_默认做法<范_该总机内所有字典_数据原项_可能的默认形态, 范_该总机内所有字典_数据视项_各形态之交集['值']>;
            任何字典_凭某数据原项构建数据视项_默认做法?: 范_任何字典_凭某数据原项构建数据视项_默认做法<范_该总机内所有字典_数据原项_可能的默认形态, 范_该总机内所有字典_数据视项_各形态之交集>;
        }
    ): (
        null | 范_字典机<范_该字典_数据原项, 范_该字典_数据视项>
    ) {
        ...
        /** 此处定义该【范】是为了在本函数体中方便指代。 */
        type 范_用以构建该字典机之配置项总集_完备形式 = 范_用以构建单个字典机之配置项总集_完备形式<范_该字典_数据原项, 范_该字典_数据视项>;

        /** 此处定义该【范】是为了在本函数体中方便指代。 */
        type 范_该字典机 = 范_字典机<范_该字典_数据原项, 范_该字典_数据视项>;
        ...
        /** 此处定义该【范】是为了在本函数体中方便指代。 */
        type 范_该字典_代表全选的字典项_完备形式 = 范_某字典_代表全选的字典项_完备形式<范_该字典_数据视项>;

        /** 此处定义该【范】是为了在本函数体中方便指代。 */
        type 范_该字典_代表全选的字典项_准用形式 = 范_某字典_代表全选的字典项_准用形式<范_该字典_数据视项>;

        /** 此处定义该【范】是为了在本函数体中方便指代。 */
        type 范_该字典_代表全都不选的字典项_完备形式 = 范_某字典_代表全都不选的字典项_完备形式<范_该字典_数据视项>;

        /** 此处定义该【范】是为了在本函数体中方便指代。 */
        type 范_该字典_代表全都不选的字典项_准用形式 = 范_某字典_代表全都不选的字典项_准用形式<范_该字典_数据视项>;
        ...
        const 应构建数据视项之哈希表 = !!配置项集.应构建数据视项之哈希表



        let 代表全选的字典项_准用形式 = 求合规的非列表对象_若不合规则给出null<范_该字典_代表全选的字典项_准用形式>(配置项集['代表全选的字典项'])
        if (代表全选的字典项_准用形式) {
            const { 应位于候选列表之该处, 数据视项 } = 代表全选的字典项_准用形式

            if (!是合规的非列表对象(数据视项)) {
                console.error(`在用以构建字典 “${该字典之唯一标识}” 的配置项总集中：【\`代表全选的字典项\`】未附带合规的【\'数据视项\'】。`)
                代表全选的字典项_准用形式 = null
            } else {
        ...
        if (代表全都不选的字典项_准用形式) {
            const { 应位于候选列表之该处, 数据视项 } = 代表全都不选的字典项_准用形式

            if (!是合规的非列表对象(数据视项)) {
                console.error(`在用以构建字典 “${该字典之唯一标识}” 的配置项总集中：【\`代表全都不选的字典项\`】未附带合规的【\'数据视项\'】。`)
        ...
        if (外界须提供函数用以凭数据原项集构建诸数据映射表) {
            if (typeof 凭数据原项集构建诸数据映射表 !== 'function') {
                if (typeof 任何字典_凭数据原项集构建诸数据映射表_默认做法 === 'function') {
                    凭数据原项集构建诸数据映射表 = 任何字典_凭数据原项集构建诸数据映射表_默认做法 as unknown as 范_任何字典_凭数据原项集构建诸数据映射表_默认做法<范_该字典_数据原项, 范_该字典_数据视项['值']>
                } else {
        ...
        let { 凭某数据原项构建数据视项 } = 事件处理程序集

        if (typeof 凭某数据原项构建数据视项 !== 'function') {
            if (typeof 任何字典_凭某数据原项构建数据视项_默认做法 === 'function') {
                凭某数据原项构建数据视项 = 任何字典_凭某数据原项构建数据视项_默认做法 as unknown as 范_任何字典_凭某数据原项构建数据视项_默认做法<范_该字典_数据原项, 范_该字典_数据视项>
            } else {
                console.error(`在用以构建字典 “${该字典之唯一标识}” 的配置项总集中：【\`凭某数据原项构建数据视项\`】和【\`任何字典_凭某数据原项构建数据视项_默认做法\`】都不是函数。`)
                return null
            }
        }
        ...
        function _凭数据原项集构建数据视项集 (): void {
            const { 凭某数据原项构建数据视项 } = 该字典机._事件处理程序集
            const { 数据原项集, 代表全选的字典项, 代表全都不选的字典项 } = 该字典机.数据集

            if (!Array.isArray(数据原项集)) { return }

            type 范_该字典_数据视项集 = 范_该字典机['数据集']['数据视项集']

            const 数据视项之列表: 范_该字典_数据视项集['列表形式'] = []
            const 数据视项之哈希表: 范_该字典_数据视项集['哈希表形式'] = {}
            ...
            该字典机.数据集.数据视项集.列表形式 = 数据视项之列表
            该字典机.数据集.数据视项集.哈希表形式 = 数据视项之哈希表
    ...
    if (typeof 任何字典_凭某数据原项构建数据视项_默认做法 !== 'function') {
        任何字典_凭某数据原项构建数据视项_默认做法 = undefined
    }
