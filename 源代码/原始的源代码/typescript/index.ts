...
> = {
    ...
    为: {
        清空数据: () => void;
        重建数据: (...用以获取字典诸条目之原式_其列表的参数表: any[]) => Promise<void>;
    ...
    为: {
        ...
        追加一批字典: <范_该组字典_条目原式_可能的默认形态 = 范_该总机内所有字典_条目原式_可能的默认形态>(
            用以构建一批字典机之配置集之列表?: Array<
                范_字典机_构建时所用配置总集_针对所属总机的准用式<范_该组字典_条目原式_可能的默认形态, 范_该总机内所有字典_条目视式_各色形态之交集>
            >
        ) => void;
...
> {
    ...
    >(
        ...
        function 重建数据 (...用以获取字典诸条目之原式_其列表的参数表: any[]): Promise<void> {
            该字典机.为.清空数据()
            ...
            return Promise.all([
                ...
                该字典机._事件处理程序集.构建或获取该字典所有条目之原式(...用以获取字典诸条目之原式_其列表的参数表).then(字典诸条目之原式_其列表 => {
                    该字典机.数据集.字典诸条目之原式_其列表 = 字典诸条目之原式_其列表
                }),
            ]).then(() => {
                该字典机._暗为._凭字典诸条目之原式_其列表构建字典条目之视式集()
        ...
        function 凭某条目之值求其呈示称谓 (所涉条目之值?: any): string | null | undefined {
