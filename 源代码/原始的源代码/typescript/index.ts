...
type 范_字典机_范之零件_事件处理程序集_针对所属总机的准用式<
    范_该字典_条目原式 = any,
    范_该字典_条目视式 extends 范_字典条目之视式_最简式 = 范_字典条目之视式_最简式<any>,
> = Partial<
...
type 范_字典机_构建时所用配置总集_针对所属总机的准用式<
    范_该字典_条目原式 = any,
    范_该字典_条目视式 extends 范_字典条目之视式_最简式 = 范_字典条目之视式_最简式<any>
> = {
    该字典之唯一标识: string;
...
> = {
    ...
    态: {
        期待_迄今所有批次的字典机之数据重建任务均已完成: null | Promise<void>;
    },
    ...
    为: {
        ...
        删除一批字典: (所涉字典唯一标识之列表?: 范_字典_唯一标识[]) => void;
...
> {
    ...
    function 删除一批字典 (所涉字典唯一标识之列表?: 范_字典_唯一标识[]): void {
        if (
            !Array.isArray(所涉字典唯一标识之列表) ||
            所涉字典唯一标识之列表.length === 0
        ) { return }
        ...
        let 字典机之新列表 = [ ...字典机之旧列表 ].filter(某字典机 => {
            const 该字典应保留 = !所涉字典唯一标识之列表.includes(某字典机.该字典之唯一标识)
    ...
    function 令所有字典按需重建数据 (
        ...
        const 期待_此批字典机之数据重建任务均已完成: Promise<void> = Promise.all(
        ...
        const {
            期待_迄今所有批次的字典机之数据重建任务均已完成: 所有批次任务均已完成之旧期待,
        } = 态



        let 期待_迄今所有批次的字典机之数据重建任务均已完成: Promise<void>

        if (所有批次任务均已完成之旧期待) {
            期待_迄今所有批次的字典机之数据重建任务均已完成 = Promise.all([
                所有批次任务均已完成之旧期待,
                期待_此批字典机之数据重建任务均已完成,
            ]).then(() => undefined)
        } else {
            期待_迄今所有批次的字典机之数据重建任务均已完成 = 期待_此批字典机之数据重建任务均已完成
        }

        期待_迄今所有批次的字典机之数据重建任务均已完成 = 期待_迄今所有批次的字典机之数据重建任务均已完成.then(() => {
            态.期待_迄今所有批次的字典机之数据重建任务均已完成 = null
        })



        态.期待_迄今所有批次的字典机之数据重建任务均已完成 = 期待_迄今所有批次的字典机之数据重建任务均已完成



        return 期待_此批字典机之数据重建任务均已完成
    }
    ...
    >(
        ...
        type 范_该字典机_构建时所用配置总集_针对所属总机的准用式 = 范_字典机_构建时所用配置总集_针对所属总机的准用式<
            范_该字典_条目原式,
            范_该字典_条目视式
        >;
        ...
        if (字典机) {
            console.warn(`不应重复构建标识为 “${该字典之唯一标识}” 的【字典机】。已略过该构建任务。`)
            return 字典机
        }

        const 事件处理程序集 = 求合规的非列表对象_若不合规则给出空的对象<
            Partial<范_字典机_范之零件_事件处理程序集_完备式<范_该字典_条目原式, 范_该字典_条目视式>>
        >(已知合规的配置总集['事件处理程序集'])

        const 最终合并采纳的配置总集: 范_该字典机_构建时所用配置总集_针对所属总机的准用式 = {
            ...已知合规的配置总集,
    ...
    function _发布消息_单个字典之数据已重建 (所涉字典机: 范_字典机<any, 范_该总机内所有字典_条目视式_各色形态之交集>): void {
    ...
    function 构建单个字典机 <
        ...
        范_该字典_条目视式 extends 范_该总机内所有字典_条目视式_各色形态之交集 = 范_该总机内所有字典_条目视式_各色形态之交集
    >(
        用以构建该字典机之配置总集?: 范_字典机_构建时所用配置总集_针对所属总机的准用式<范_该字典_条目原式, 范_该字典_条目视式>,
        ...
        type 范_该字典机_事件处理程序集_针对所属总机的准用式 = 范_字典机_范之零件_事件处理程序集_针对所属总机的准用式<
            范_该字典_条目原式,
            范_该字典_条目视式
        >;
        ...
        const 已知合规的配置总集 = 求合规的非列表对象_若不合规则给出null<范_该字典机_构建时所用配置总集_针对所属总机的准用式>(用以构建该字典机之配置总集)
        if (!已知合规的配置总集) {
            // 不必给出警示或出错提示。
            return null
        }
        ...
        if (!是字典之合规的唯一标识(该字典之唯一标识)) {
            console.error('用以构建某字典之配置总集中：缺少合规的【\`该字典之唯一标识\`】。所涉配置总集为：', 用以构建该字典机之配置总集)
            return null
        }

        const 事件处理程序集 = 求合规的非列表对象_若不合规则给出null<范_该字典机_事件处理程序集_针对所属总机的准用式>(已知合规的配置总集['事件处理程序集'])
        if (!事件处理程序集) {
        ...
        if (typeof 构建或获取该字典所有条目之原式 !== 'function') {
            console.error(`在用以构建字典 “${该字典之唯一标识}” 的配置总集中：【\`构建或获取该字典所有条目之原式\`】不是函数。`)
            return null
        }
        ...
        if (字典特别条目_其代表全选者_准用式) {
            ...
            if (!是合规的非列表对象(字典条目之视式)) {
                console.error(`在用以构建字典 “${该字典之唯一标识}” 的配置总集中：【\`字典特别条目_其代表全选者\`】未附带合规的【\'字典条目之视式\'】。`)
        ...
        if (字典特别条目_其代表全都不选者_准用式) {
            ...
            if (!是合规的非列表对象(字典条目之视式)) {
                console.error(`在用以构建字典 “${该字典之唯一标识}” 的配置总集中：【\`字典特别条目_其代表全都不选者\`】未附带合规的【\'字典条目之视式\'】。`)
        ...
        if (外界须提供函数用以脱离字典条目原式之集构建诸字典条目内容映射表) {
            if (typeof 脱离字典条目原式之集构建诸字典条目内容映射表 !== 'function') {
                console.error(`在用以构建字典 “${该字典之唯一标识}” 的配置总集中：【\`脱离字典条目原式之集构建诸字典条目内容映射表\`】不是函数。`)
                return null
            }
        }
        ...
        if (外界须提供函数用以凭字典条目原式之集构建诸字典条目内容映射表) {
            if (typeof 凭字典条目原式之集构建诸字典条目内容映射表 !== 'function') {
                if (typeof 任何字典_凭字典条目原式之集构建诸字典条目内容映射表_默认做法 === 'function') {
                    ...
                    console.error(`在用以构建字典 “${该字典之唯一标识}” 的配置总集中：【\`凭字典条目原式之集构建诸字典条目内容映射表\`】和【\`任何字典_凭字典条目原式之集构建诸字典条目内容映射表_默认做法\`】都不是函数。`)
                    return null
                }
            }
        }
        ...
        if (typeof 凭某条目之原式构建其视式 !== 'function') {
            if (typeof 任何字典_凭某条目之原式构建其视式_默认做法 === 'function') {
                ...
                console.error(`在用以构建字典 “${该字典之唯一标识}” 的配置总集中：【\`凭某条目之原式构建其视式\`】和【\`任何字典_凭某条目之原式构建其视式_默认做法\`】都不是函数。`)
                return null
            }
        }
        ...
        function _凭字典诸条目之原式_其列表构建字典条目之视式集 (): void {
        ...
        function _按需构建诸字典条目内容映射表 (): Promise<void> {
            ...
            if (该字典构建诸字典条目内容映射表之方案 === '从该字典条目之原式之集提取而得') {
                ...
                if (!字典诸条目之原式_其列表) {
                    console.error(`字典 “${该字典之唯一标识}” 的诸【字典条目内容映射表】的构建方案配置为 “${该字典构建诸字典条目内容映射表之方案}”。但构建诸【字典条目内容映射表】时，发现【\`字典诸条目之原式_其列表\`】尚不可用。`)
                    return 构建诸字典条目内容映射表一事已落实
                    // return Promise.reject()
                }

                if (typeof 凭字典条目原式之集构建诸字典条目内容映射表 !== 'function') {
                    console.error(`字典 “${该字典之唯一标识}” 的诸【字典条目内容映射表】的构建方案配置为 “${该字典构建诸字典条目内容映射表之方案}”。但构建诸【字典条目内容映射表】时，发现【\`凭字典条目原式之集构建诸字典条目内容映射表\`】不是函数。`)
                    return 构建诸字典条目内容映射表一事已落实
                    // return Promise.reject()
                }
            ...
            if (
                ...
                if (typeof 脱离字典条目原式之集构建诸字典条目内容映射表 !== 'function') {
                    console.error(`字典 “${该字典之唯一标识}” 的诸【字典条目内容映射表】的构建方案配置为 “${该字典构建诸字典条目内容映射表之方案}”。但构建诸【字典条目内容映射表】时，发现【\`脱离字典条目原式之集构建诸字典条目内容映射表\`】不是函数。`)
                    return 构建诸字典条目内容映射表一事已落实
                    // return Promise.reject()
                }
            ...
            构建诸字典条目内容映射表一事已落实 = 构建诸字典条目内容映射表之主体一事已落实.then(function 在映射表中按需补充特别条目 () {
                ...
                if (字典特别条目_其代表全选者) {
                    ...
                    if (值 in 从值至呈示称谓) {
                        console.warn(`字典 “${该字典之唯一标识}” 的【字典条目内容映射表】中已有称谓为 “${从值至呈示称谓[值]}” 的条目取值为 “${值}”，故不会将称谓 “${呈示称谓}” 映射到该值。`)
                    } else {
                        从值至呈示称谓[值] = 呈示称谓
                    }

                    if (呈示称谓 in 从呈示称谓至值) {
                        console.warn(`字典 “${该字典之唯一标识}” 的【字典条目内容映射表】中已有值为 “${值}” 的条目，其称谓为 “${从值至呈示称谓[值]}”，故不再将该值映射到称谓 “${呈示称谓}” 。`)
                    } else {
                        从值至呈示称谓[呈示称谓] = 值
                    }
                }
        ...
        function 重建数据 (...用以获取字典诸条目之原式_参数表: any[]): Promise<void> {
            ...
            if (
                ...
                独立构建诸字典条目内容映射表一事已落实 = _暗为._按需构建诸字典条目内容映射表()
            } else {
            ...
            const 期待_本次重建数据完毕: Promise<void> = Promise.all([
                独立构建诸字典条目内容映射表一事已落实,
