...
-   任何一个【字典机】有以下主要任务或功能：
    ...
    1.  产生或取得所有【条目】之【原式】。且该任务可以按需反复执行，以令该【字典】按需反复重建一切数据。

        > 实践中，不同的【字典】构建其【条目】之【原式】的办法各不相同。多见是凭借某 XHR 过程获取。当然也可以由写死的代码直接生成。总之，任何【字典】均可以订制其构建一切【条目】之【原式】的办法。
        >
        > 参见下文《[【字典群之总机】之功能及用法](#字典群之总机之功能及用法)》中有关制作【字典机】的内容。


    1.  保有并对外提供所有【条目】之【原式】。若有必要，还额外保有两个特殊【条目】，分别代表“全选”和“全都不选”。
...
1.  可制作一批新【字典机】，并追加到该【总机】所辖的【字典机】集中。此亦即制作了一批新【字典】。
    ...
    1.  因为【字典机】须构建两种【条目内容映射表】对象，所以在创建任何一个【字典机】时：
        ...
        1.  当该【字典机】不是所谓【标准字典】，且该【字典机】的 `形制杂项集` 的 `该字典构建诸字典条目内容映射表之方案` 取文本值 `'脱离该字典之诸条目原式之集而构建，仅一次即可'`, 或文本值 `'脱离该字典之诸条目原式之集而构建，且须反复构建'`, 时，该【字典机】必须给出 `脱离字典条目原式之集构建诸字典条目内容映射表` 函数。

        1.  当该【字典机】不是所谓【标准字典】，且该【字典机】的 `形制杂项集` 的 `该字典构建诸字典条目内容映射表之方案` 取文本值 `'从该字典之诸条目原式之集提取而得'` 时，该【字典机】必须给出 `凭字典条目原式之集构建诸字典条目内容映射表` 函数。
