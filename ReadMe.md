...
由上述两例可见，虽然某给定的【字典】内一切【条目】均须采用完全相同的数据形态，但**于不同的两个【字典】**，其【条目】之数据形态可能迥异，或则说，二者内所谓【原式】可能迥异。

任凭多个【字典】之【条目】之【原式】可能迥异，每当将这些【字典条目】运用在用户界面代码（例如 Vuejs 的模板）中时，我们希望一切【字典】的一切【条目】均有统一的格式。一旦统一，既可提高界面代码的可重用性，又可减少因人为疏忽而造成的软件故障。由此，鄙人提出所谓【视式】的概念。
...
1. #### 术语【形制要求】、【形制要求集】与【形制项集】
    ...
    在传统的编程劳动中，习惯采用外国语 `option` 或其复数 `options` 、或者 `config` 、 `configuration` 、 `settings` 等词来表达“可配置项”、“选项”之类的含义。本工具集及其配套文档则采用【形制要求】一词表述它们。并且，在为相关常量、变量命名时，采用【`形制要求集`】或其变形（如【`形制要求总集`】、【`形制要求杂集`】、【`形制杂集`】和【`形制杂项集`】）等汉语短语来代替它们。
...
1. #### 术语【`还应持有`】与【`所持`】
    ...
    在传统的编程劳动中，习惯采用外国语 `data` 一词表达“数据”之含义。偶见采用其单数形式 `datum` 。本工具集及其配套文档则采用【`还应持有`】和【`所持`】两个词代替它们。该二词既用于本文描述，也用于各属性、变量之命名。

    一般的，在某函数的**参数**中数据配置，为其命名时鄙人倾向于采用【`还应持有`】一词。

    又，在**所谓“实用”对象**中数据配置，为其命名时鄙人倾向于采用【`所持`】一词。
...
1. #### 术语【`诸事之应对`】与【`应对`】
    ...
    此二者既用于本文描述，也用于各属性、变量之命名。
    
    一般的，在某函数的**参数**中事件处理函数集，为其命名时鄙人倾向于采用【`诸事之应对`】一词。
    
    又，在**所谓“实用”对象**中事件处理函数集，为其命名时鄙人倾向于采用【`应对`】一词。
...
1. #### 术语【标准字典】与【特异字典】
    ...
    **由此还可以看出，虽然本工具集创造了所谓【标准字典】之概念，但鄙人并没有在【字典机】的【形制要求集】中设计任何真实的字段用以标记或表征该【字典机】之【字典】是否为所谓【标准字典】。一个【字典】是否为所谓【标准字典】，仅“暗中”取决于它是否采用【总机】提供的“默认做法”。** 通俗地讲，在 JavaScript 层面（而非 TypeScript 层面，见下文）没有任何字段用以区别所谓【标准字典】和【特异字典】。二者完全由开发者内心界定。
