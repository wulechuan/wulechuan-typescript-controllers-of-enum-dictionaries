...
1.  可制作一批新【字典机】，并追加到该【总机】所辖的【字典机】集中。此亦即制作了一批新【字典】。
    ...
    其中实参 `一批字典机之形制要求总集之列表` 是一个列表（Array）。该列表的每个成员都是一个【字典机】的构造形制之要求。该形制要求之【范】较冗长，不便阅读、理解。故不妨先给出常量形式。常量形式相对完整定义显得较为简略，可以辅助观察和理解。常量形式如下。
